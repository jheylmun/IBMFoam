    ...
    centeredAxis_(nk_, Zero)
{
    ...
    p1_ = vector(center.x(), center.y(), bb.min().z());
    p2_ = vector(center.x(), center.y(), bb.max().z());
    ...
    if (nk_ == 1)
    {
        ...
        centeredAxis_[0] - axis_[0] - this->center_;
    }
    else
    {
        forAll(axis_, k)
        {
            ...
            centeredAxis_[k] = axis_[k] - this->center_;
        }
    }
...
Foam::scalar Foam::particleShapes::cylinder::A() const
{
...
void Foam::particleShapes::cylinder::moveMesh(const vector& center)
{
    this->center_ = center;

    forAll(baseMesh_, celli)
    {
        baseMesh_[celli] = centeredMesh_[celli] + this->center_;
    }
    forAll(axis_, k)
    {
        axis_[k] = centeredAxis_[k] + this->center_;
    }
}
